...
// The idea is that by running the test create_publish_module
// `cargo test create_publish_module` in `smoke-test`
// a rust file (`generic_module.rs`) gets generated in
// `module_publishing/build` which contains a `CompiledModule`
...
module 0xABCD::Simple {
    ...
    public entry fun step(_s: &signer) acquires Counter {
        let counter = borrow_global_mut<Counter>(@0xABCD);
        *(&mut counter.count) = counter.count + COUNTER_STEP;
        assert!(false, 100);
    }

    // Get the value behind `Counter`.
    public entry fun get_counter(_s: &signer) acquires Counter {
        let counter = borrow_global_mut<Counter>(@0xABCD);
        counter.count;
    }
