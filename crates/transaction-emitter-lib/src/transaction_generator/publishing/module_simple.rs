...
use move_binary_format::{file_format::{IdentifierIndex, SignatureToken}, CompiledModule};
...
pub fn load_package() -> (Vec<CompiledModule>, PackageMetadata) {
    let metadata = bcs::from_bytes::<PackageMetadata>(
        &raw_module_data::PACKAGE_METADATA_SIMPLE
    ).expect("PackageMetadata for GenericModule must deserialize");
    let mut modules = vec![];
    let module = CompiledModule::deserialize(
        &raw_module_data::MODULE_SIMPLE
    ...
    if let Some(fd) = def {
        for suffix in 0..fn_count {
            ...
            module.identifiers.push(
                Identifier::new(name.as_str()).expect("Identifier name must be valid")
            );
...
pub fn zero_args_function(rng: &mut StdRng, module_id: ModuleId) -> TransactionPayload {
    ...
    match EntryPoints::try_from(fun_idx).expect("Must pick a function in range, bogus id generated") {
        ...
        EntryPoints::GetCounter => get_payload_void(module_id, ident_str!("get_counter").to_owned()),
        ...
        _ => panic!("bogus zero_args_function id generation, check beginning of function"),
    }
}
...
pub fn rand_gen_function(rng: &mut StdRng, module_id: ModuleId) -> TransactionPayload {
    ...
    info!("PublisPackage: in rand_gen_function {}", fun_idx);
    match EntryPoints::try_from(fun_idx).expect("Must pick a function in range, bogus id generated") {
        // 0 args
        EntryPoints::Nop => get_payload_void(module_id, ident_str!("nop").to_owned()),
        EntryPoints::Step => get_payload_void(module_id, ident_str!("step").to_owned()),
        ...
        _ => panic!("bogus rand_gen_function id generation, check beginning of function"),
    }
}

pub fn any_function(rng: &mut StdRng, module_id: ModuleId, other: AccountAddress) -> TransactionPayload {
    ...
    match EntryPoints::try_from(fun_idx).expect("Must pick a function in range, bogus id generated") {
        ...
        EntryPoints::Maximize => maximize(module_id, other),
        EntryPoints::Minimize => minimize(module_id, other),
        // 3 args
...
fn set_name(rng: &mut StdRng, module_id: ModuleId) -> TransactionPayload {
    ...
    let name: String = rng.sample_iter(&Alphanumeric)
        .take(len)
        .map(char::from)
        .collect();
    get_payload(
