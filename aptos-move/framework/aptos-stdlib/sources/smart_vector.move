module aptos_framework::smart_vector {
    ...
    /// metadata associated with BigVector when smart_vector is so small that inline_inner vector can hold all the data.
    ...
    fun smart_vector_remove_test() {
        ...
        remove(&mut v, 30);
        remove(&mut v, 20);
        assert!(vector::length(&v.inline_inner) == inline_len, 0);
        remove(&mut v, 10);
        assert!(vector::length(&v.inline_inner) + 1 == inline_len, 0);
        remove(&mut v, 0);
        assert!(vector::length(&v.inline_inner) + 2 == inline_len, 0);
    ...
    fun smart_vector_reverse_test() {
        ...
        let k = 0;
        while (k < 10) {
            assert!(*vector::borrow(&v.inline_inner, k) == 9 - k, 0);
            k = k + 1;
        };
        while (i < 100) {
            push_back(&mut v, i);
        ...
        while (!vector::is_empty(&v.inline_inner)) {
            remove(&mut v, 0);
        };
        reverse(&mut v);
        i = 0;
        let len = length(&v);
        while (i + 1 < len) {
            assert!(*big_vector::borrow(vector::borrow(&v.table_inner, 0), i) == *big_vector::borrow(vector::borrow(&v.table_inner, 0), i + 1) + 1, 0);
            i = i + 1;
        };
        destroy(v);
    }
