/// This module is the step 1 of our NFT Move tutorial. In this module, we go over how to create a collection and token,
...
/// Run the following command in directory `aptos-core/aptos-move/move-examples/mint_nft/step1`.
...
/// example output:
    /*
    step1 % aptos move publish --named-addresses mint_nft=a911e7374107ad434bbc5369289cf5855c3b1a2938a6bfce0776c1d296271cde
...
///
/// 3. Check out the `mint_nft()` function below - we are not going to run a command to mint the NFT in this step, because
/// this function right now asks for two signers and that's inpractical to do using CLI commands.
/// In the next step of this tutorial, we will introduce a way to programmatically sign for transactions, so the module publisher
/// doesn't need to manually sign transactions, and only needs one signer (the nft receiver's signer) for the `mint_nft()` function.
module mint_nft::nft_step1 {
    use std::bcs;
    use std::error;
    use std::signer;
    ...
    /// In step 2 of this tutorial, we will introduce the concept of "resource account" - it is
    ...
    public entry fun mint_nft(module_owner: &signer, receiver: &signer) acquires ModuleData {
