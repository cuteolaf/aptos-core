/// This module is the step 2 of our NFT Move tutorial, building on top of step 1. In this step of the tutorial, we introduce the concept of a resource
...
/// 1. Create an nft-receiver account (in addition to the source account we created in the last step). We'll use this account to receive an NFT in this tutorial.
/// aptos init --profile nft-receiver
///
/// 2. Publish the module under a resource account.
/// - 2.a Make sure you're in the right directory.
/// Run the following command in directory `aptos-core/aptos-move/move-examples/mint_nft/step2`
...
/// example output:
    /*
    step2 % aptos move create-resource-account-and-publish-package --seed 1235 --address-name mint_nft --profile default --named-addresses source_addr=a911e7374107ad434bbc5369289cf5855c3b1a2938a6bfce0776c1d296271cde
...
/// What if we want to update the configuration of this module? In the next step of this tutorial, we will go over how to add an admin account and admin functions
...
/// - 3.c In `mint_nft`, we create a resource signer by calling `account::create_signer_with_capability(&module_data.signer_cap)` to programmatically sign for `token::mint_token()` and `token::direct_transfer()` functions.
...
/// aptos move run --function-id [resource account's address]::nft_step2::mint_nft --profile nft-receiver
///
/// example output:
    /*
    step2 % aptos move run --function-id 55328567ff8aa7d242951af7fc1872746fbeeb89dfed0e1ee2ff71b9bf4469d6::nft_step2::mint_nft --profile nft-receiver
...
module mint_nft::nft_step2 {
    ...
    fun init_module(resource_signer: &signer) {
        ...
        // sign for transactions in the `mint_nft()` function.
    ...
    /// Mint an NFT to the receiver. Note that different from the tutorial in step 1, here we only ask for the receiver's
