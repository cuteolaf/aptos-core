/// This module is the last step of our NFT Move tutorial. In this step, we are fleshing out the smart contracts so it
/// becomes production-ready. Specifically, in this step we are going to go over:
...
/// authentication key.
///
/// In this tutorial, we are using it for a different reason: we would like to make sure that only mint_nft() requests
...
/// Go to `Move.toml` and replace `admin_addr = "0xbeef"` with the admin address we created in step 3.
///
/// 2. Publish the module under a resource account.
/// - 2.a Make sure you're in the right directory.
/// We should run the following command in directory `aptos-core/aptos-move/move-examples/mint_nft/step4`.
...
///
/// 3. Call `mint_nft()` with a valid signature to mint a token.
...
/// aptos move run --function-id [resource account's address]::step4::set_public_key --args hex:[public key we just generated] --profile admin
/// (To find the public key we just generated, run `nano output.key.pub`.)
/// example output:
    /*
    step4 % aptos move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::step4::set_public_key --args hex:5C0637A3865FCA80550502BC30C8E7B4CCA7C8AB3B4FFECEFC8C43F7D0D44DEE --profile admin
...
///  In function `generate_nft_tutorial_step4_signature`, change the `resource_address`, `nft_receiver`, `admin_private_key`, and `receiver_account_sequence_number` variables to the actual values.
...
///  Run `cargo test generate_nft_tutorial_step4_signature -- --nocapture` to generate a valid signature that we'll use in the next step.
/// - 3.d Call mint_nft() with the signature we generated in the last step.
/// aptos move run --function-id [resource account's address]::step4::mint_nft --args hex:[signature generated in last step] --profile nft-receiver
/// example output:
    /*
    step4 % aptos move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::step4::mint_nft --args hex:fc833512ad1c575850569d14f5e434b929a19eb491c08df9f6b91584a13551bdb95830081a429f148fddcb9ba201cf72a357957849046da0d60675ed034f580f --profile nft-receiver
