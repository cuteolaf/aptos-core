/// This module is the step 3 of our NFT Move tutorial, building on top of step 2. In this module, we introduce an admin account and
...
///
/// In this step of the NFT tutorial, we are adding an admin account and two admin functions on top of the previous tutorial, so we can set
/// and update when/if we want to enable token minting for this contract.
///
/// How to interact with this module:
/// 1. Create and configure an admin account (in addition to the source account and nft-receiver account that we created in the earlier steps).
...
/// Run the following command in directory `aptos-core/aptos-move/move-examples/mint_nft/step3`.
...
/// example output:
    /*
    step3 % aptos move create-resource-account-and-publish-package --seed 1239 --address-name mint_nft --profile default --named-addresses source_addr=a911e7374107ad434bbc5369289cf5855c3b1a2938a6bfce0776c1d296271cde
...
/// - 3.c In `mint_nft()`, we added two assert statements to make sure that the user can only mint token from this collection if minting is enabled and
...
/// aptos move run --function-id [resource account's address]::nft_step3::mint_nft --profile nft-receiver
/// example output:
    /*
    step3 % aptos move run --function-id 34f5acaaef16988aa31bb56ad7b35e30d14ff9fc849d370be799617b61d3df04::nft_step3::mint_nft --profile nft-receiver
    {
      "Error": "Simulation failed with status: Move abort in 0x34f5acaaef16988aa31bb56ad7b35e30d14ff9fc849d370be799617b61d3df04::nft_step3: EMINTING_DISABLED(0x50003): The collection minting is disabled"
    }
    */
...
/// aptos move run --function-id [resource account's address]::nft_step3::set_minting_enabled --args bool:true --profile admin
/// example output:
    /*
    step3 % aptos move run --function-id 34f5acaaef16988aa31bb56ad7b35e30d14ff9fc849d370be799617b61d3df04::nft_step3::set_minting_enabled --args bool:true --profile admin
...
module mint_nft::nft_step3 {
    ...
    /// Mint an NFT to the receiver. Note that different from the tutorial in step1, here we only ask for the receiver's
